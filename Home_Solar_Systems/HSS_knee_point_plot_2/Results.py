...
def Plot_Energy_Total(instance, Time_Series, plot, Plot_Date, PlotTime):  
    ...
    if plot == 'No Average':
        ...
        Fill['LostLoad'] = Fill[b]
        
        
        for i in Fill.index:
            
            if Fill[b][i]> Fill[d][i]:
                Fill.loc[i,b] = Fill[d][i]
...
#        Fill[b] = ( Fill[g] + Plot_Data[b])
        ...
        ax3.fill_between(Fill.index, Fill[g].values, Fill[b].values,   
                    ...
                    ,   where=Fill[g].values<=Fill[d].values, interpolate =True
                    )
        # Demand
        ...
        if instance.Lost_Load_Probability > 0:
            ...
            hatch_LL = '-'
            C_LL = 'crimson'
            ax4.fill_between(Fill.index,Fill['LostLoad'].values, Fill[d].values, 
                             alpha=alpha_LL, color=C_LL,edgecolor=  C_LL, 
                             hatch =hatch_LL, where=Fill['LostLoad'].values<=Fill[d].values
                             , interpolate =True
                         ) 
