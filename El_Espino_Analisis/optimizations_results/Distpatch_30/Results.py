...
def Load_results1_Dispatch(instance):
    ...
    Battery_Usage = round(Battery_Usage*100, 1)
    ...
    Usage_Renewable = (Renewable_Energy_Penetration_Data['Renewable Energy (kWh)'].sum() 
                       - Renewable_Energy_Penetration_Data['Curtailment (kWh)'].sum())
    Percentage_Renewable = Usage_Renewable/Scenarios['Energy Demand (kWh)'].sum()
    Percentage_Renewable = round(Percentage_Renewable*100, 1)
    print('The uncurtail renewable energy represents ' + str(Percentage_Renewable) + ' % of the total demand')
...
#                print(name)
 ...
 #       ax1.tick_params(axis='x', which='major', labelsize = tick_size) 
        ...
        for R in range(1, Renewable_Source + 1):
            if R == 1:
                From_Renewable.append(mpatches.Patch(color='yellow',alpha=Alpha_r, label='PV Energy'))  
            else:
                name = 'From Renewable ' +str(R) 
                c_r_1 = c_r[R-1] 
                foo = mpatches.Patch(color=c_r_1,alpha=Alpha_r, label=name)
