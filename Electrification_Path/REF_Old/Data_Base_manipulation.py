...
PV_2012 = pd.read_csv('Bolivia/PV_Total_2012.csv', index_col=0)  

data['PV total output'] = PV_2012['Total PV Energy']
data.loc[16141, 'PV total output'] = 500


#%%

GHI_2012 = pd.read_csv('Bolivia/GHI_Total_2012.csv', index_col=0)
...
test['GHI'] = data['GHI']
test['Total GHI Energy'] = GHI_2012['Total GHI Energy']
test['diff'] = abs(test['GHI'] -  test['Total GHI Energy']) 
rate_ghi = test['GHI'].sum()/test['Total GHI Energy'].sum()
print(round(rate_ghi,2))

test_describe = test.describe()


data['GHI'] = GHI_2012['Total GHI Energy']
