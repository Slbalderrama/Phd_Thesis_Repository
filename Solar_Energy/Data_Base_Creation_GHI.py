...
Number_Scenarios = 6
solar_energy = 'Variable' # 'Fix' 'Variable'
sleep = 15
...
for j in Data_Villages.index[:]:
    ...
    if solar_energy=='Variable': 
        name = 'Variable_Energy_GHI/GHI_Energy_' +  str(j) + '.csv'
        Data.to_csv(name)
       

    elif solar_energy=='Fix':
                   
        Data_Base_Solar[1] = round(Data[i+1],3)
        time.sleep(sleep)
        Data_Base_Solar.to_csv('Fix_Energy_GHI/GHI_Fix_' + str(j)+'.csv')
        print(len(Data_Base_Solar))
