...
for i in range(50,570,50):
    for n in range(150):
        ...
        data.loc[name, 'Renewable Capacity'] = Data_Renewable['Source 1']['Total Nominal Capacity (kW)']         
        ...
        data.loc[name, 'Battery Capacity'] = Data_Battery['Battery']['Nominal Capacity (kWh)']   
#        Battery_Discharge_Rate = Data_Battery['Battery']['Nominal Capacity (Wh)'] 
        ...
        data.loc[name, 'Max Demand'] = Data_Time_series['Energy Demand 1 (kWh)'].max() 
        data.loc[name, 'Mean Demand'] = Data_Time_series['Energy Demand 1 (kWh)'].mean() 
        data.loc[name, 'Total Demand'] = Data_Time_series['Energy Demand 1 (kWh)'].sum()
        
        Renewable_Energy = Data_Time_series['Renewable Energy 1 (kWh)'].sum() 
        Generator_Energy = Data_Time_series['Gen energy 1 (kWh)'].sum() 
        ...
        Curtailment = Data_Time_series['Curtailment 1 (kWh)'].sum()
        data.loc[name, 'Curtailment Percentage'] = (Curtailment/(Renewable_Energy+Generator_Energy))*100
        
        
        Battery_Energy = Data_Time_series['Battery Flow Out 1 (kWh)'].sum() 
        ...
        data.loc[name, 'Renewable Energy Unit Total'] = Data_Renewable_series['Renewable unit 1 1 (kWh)'].sum()
        ...
        data.loc[name, 'Renewable Unitary Invesment Cost'] = Data_Renewable['Source 1']['Investment Cost (USD/kW)']
        data.loc[name, 'Battery Unitary Invesment Cost'] = Data_Battery['Battery']['Unitary Invesment Cost (USD/kWh)']
        ...
        data.loc[name, 'GenSet Unitary Invesment Cost'] = Data_Generator['Generator 1']['Generator Invesment Cost (USD/kW)']  
        data.loc[name, 'Generator Efficiency'] = Data_Generator['Generator 1']['Generator Efficiency']   
        data.loc[name, 'Low Heating Value'] = Data_Generator['Generator 1']['Low Heating Value (kWh/l)']  
        data.loc[name, 'Fuel Cost'] = Data_Generator['Generator 1']['Fuel Cost (USD/l)']           
        data.loc[name, 'Generator Nominal capacity'] = Data_Generator['Generator 1']['Generator Nominal Capacity (kW)']   
